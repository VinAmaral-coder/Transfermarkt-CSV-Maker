...
async function getTeams(url) {
  ...
  $(".items tbody tr").each((i, el) => {
    const team = $(el).find("td.hauptlink a").first();
...
async function getPlayers(team) {
  ...
  $(".items tbody tr").each((i, el) => {
    const player = $(el)
      .find('a[href*="/profil/spieler/"]')
      .first();

    const playerLink = player.attr("href");

    if (!playerLink) return;

    const id = playerLink.match(/spieler\/(\d+)/)?.[1];
